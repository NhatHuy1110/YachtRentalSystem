...
<?php $login_client = $_SESSION['login_client']; 
    ...
    $sql1 = "SELECT * FROM rentedyachts ry, clientyachts cy, customers c, yachts WHERE cy.client_username = '$login_client' AND cy.yacht_id = ry.yacht_id AND ry.return_status = 'R' AND c.customer_username = ry.customer_username AND cy.yacht_id = yachts.yacht_id";
...
<td><?php  if($row["charge_type"] == "day"){
                    echo ($row["quantity"] . "(day)");
                } else {
                    echo ($row["quantity"] . "(hour)");
                } ?></td>
<td><?php  if($row["total_amount"] == NULL){
                    echo ("NULL");
                } else {
                    echo ($row["total_amount"] . "M");
                } ?></td>
</tr>
<?php        } ?>
                </table>
        ...
        <div class="container">
      <div class="jumbotron">
        <h1>No booked Yachts</h1>
        <p> Rent some Yachts now <?php echo $conn->error; ?> </p>
      </div>
    </div>
